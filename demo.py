...
def main(args):
    ...
    dirs = glob("../thesis/dataset/*")
    for dr in dirs[0:1]:
        for i in range(2):
            # ------------- load data
            # image_folder = args.inputDir
            # save_folder = args.outputDir
            image_folder = dr + '/positive' if (i==0) else dr + '/negative'
            save_folder = dr + '/prnet/positive' if (i==0) else dr + '/prnet/negative'
            ...
            for i, image_path in enumerate(image_path_list):
                ...
                if args.isPose or args.isShow:
                    # estimate pose
                    camera_matrix, rotation_vector, translation_vector = estimate_pose(vertices)
                    np.savetxt(os.path.join(save_folder, name + '_rvec.txt'), rotation_vector)
                    np.savetxt(os.path.join(save_folder, name + '_tvec.txt'), translation_vector) 
